...
class Captcha{
    ...
    private $codeNum;
    private $config=array('level'=>10,"isNoise"=>true,'simple'=>false);//干扰级别，是否有干扰元素，修改背景模糊度
    public function __construct($_width=80,$_height=28,$_codeNum=4){
        $this->width=$_width;
        $this->height=$_height;  
    ...
    //字体文件在此
    public function showCaptcha($_fontFile='ARIALNB.TTF'){
        //创建图片
        $this->createImage();
        //设置噪音:点和弧线
        $this->setNoise();
    ...
    private function createImage(){
        ...
        if($this->config['simple']){
            ...
            //设置背景色
            $bgColor=imagecolorallocate($this->image,225,255,255);
        }else{
            //设置背景色
